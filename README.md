...
- **飞书 / 企业微信双平台同时在线**：同一进程两条长连接；`launcher_windows/MyClaw-Setup.bat` / `launcher_macos/MyClaw-Setup.command` 一站式安装并选择平台，`MYCLAW_CHANNELS` 留空时按凭据自动探测，会话按平台隔离互不干扰；
...
launcher_windows/ Windows 客户入口（安装/配置/启动/重启）
