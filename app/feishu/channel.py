"""飞书 Channel 实现：包装现有 FeishuClient + 卡片构建。
...
logger = logging.getLogger("myclaw.feishu.channel")
...
class FeishuProgressHandle:
    ...
    def _should_skip(self, snap: ProgressSnap, now: float) -> bool:
        if snap.status != self._last_status:
            return False
        # 状态未变时节流：告警风暴 2s，其余 0.5s（沿用原 cli_loop 参数）
...
class FeishuChannel:
    ...
    async def send_card(self, target: UserTarget, card: dict) -> Any:
        """发原生飞书卡片（选择卡等仍由 feishu 侧构建，P3 抽 Selection）。"""
        if target.is_group and target.chat_id:
    ...
    # ---- 以下能力在后续阶段接入（P5 生命周期 start/stop/health）----

    async def send_selection(self, target: UserTarget, sel: Selection) -> None:
        # 选择卡已由 send_view 的 kind 体系覆盖，此方法保留给协议兼容
    ...
    async def send_view(self, target: UserTarget, kind: str, payload: dict) -> None:
        ...
        if kind == "claude_dir_setup":
            card = fc.build_claude_dir_selection_card(payload["detected_path"])
            ...
            card = fc.build_profile_selection_card(
            ...
            card = fc.build_balance_card(payload["result"])
