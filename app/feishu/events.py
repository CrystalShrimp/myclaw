...
def on_card_action(event: P2CardActionTrigger) -> P2CardActionTriggerResponse:
    ...
    if card_type == "claude_dir_select":
        if act == "set_detected":
            path_str = action.value.get("path", "")
        else:
            path_str = ""
            if action.form_value:
                path_str = action.form_value.get("claude_data_dir") or ""
        try:
            selections.handle_claude_dir_setup(target, path_str)
    ...
    # Model level selection card
    if card_type == "model_selection" and act == "switch_model":
        msg = selections.handle_level_switch(
            target, approval_id, action.value.get("model", ""),
        )
        return _toast("info", msg)

    # Effort selection card
    if card_type == "effort_selection" and act == "switch_effort":
        msg = selections.handle_effort_switch(target, action.value.get("effort", ""))
        return _toast("info", msg)

    # Provider profile selection card.
    if card_type == "profile_switch" and act == "switch_profile":
        ttype, msg = selections.handle_profile_switch(
            target, skey, action.value.get("profile", ""),
        )
        return _toast(ttype, msg)
    ...
    if card_type == "session_select" and act == "resume_session":
        ...
        try:
            msg = selections.handle_session_resume(target, skey, target_sid)
        except SelectionError as e:
            return _toast("error", e.message)
