...
logger = logging.getLogger("myclaw.wecom.channel")
...
class WeComChannel:
    ...
    async def send_view(self, target: UserTarget, kind: str, payload: dict) -> None:
        ...
        if kind == "help":
            ...
            for s in sessions[:15]:
                ...
                try:
                    ...
                    ts = "?"
                options.append((s.get("session_id", ""), f"{s.get('last_summary', '')} · {ts}"))
            _numbered("选择要恢复的 Session", options, footer="或直接发送 `/resume <session_id>`")
            await self._respond_or_push(
                target, self._client.respond_markdown,
                wc.numbered_markdown("选择要恢复的 Session", options, "或直接发送 `/resume <session_id>`"),
            )
            ...
            desc = (
                ...
                f"Git: {payload.get('git_branch', '-')} | CLAUDE.md: {payload.get('claude_md', '-')}"
            )
        ...
        elif kind == "claude_dir_setup":
            detected = payload.get("detected_path", "")
            _numbered("设置 Claude 数据目录", [(detected, f"使用检测到的路径 `{detected}`")])
            await self._respond_or_push(
                target, self._client.respond_markdown,
                wc.numbered_markdown(
                    "设置 Claude 数据目录",
                    [(detected, f"使用检测到的 `{detected}`")],
                    "未检测到有效目录时，请先在本机安装 Claude Code 并运行一次，再重启 MyClaw。",
                ),
            )
