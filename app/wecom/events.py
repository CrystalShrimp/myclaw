...
logger = logging.getLogger("myclaw.wecom.events")
...
async def _run_selection(
    ...
    if kind == "claude_dir_setup":
        selections.handle_claude_dir_setup(target, value)
        return None
    ...
    if kind == "provider_selection":
        ttype, msg = selections.handle_profile_switch(target, skey, value)
        return msg
        ...
        try:
            if _CHANNEL._client is not None:
                ...
                await _CHANNEL._client.respond_welcome(
                    req_id, "我是 MyClaw，发送 /help 查看指令，或直接发任务给我。",
                )
...
def _parse_and_dispatch(target: UserTarget, raw_key: str) -> str | None:
    ...
    if action == "lv":
        return selections.handle_level_switch(target, extra, value)
    if action == "ef":
