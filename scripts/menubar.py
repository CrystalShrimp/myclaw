...
行为：菜单栏图标实时反映后端健康状态；点击菜单可查看状态（含飞书/企微
通道连接情况）、打开日志、退出（退出时停止后端子进程）。
"""
...
def start_server() -> None:
    ...
    if not python.exists():
        print(f"[ERROR] 未找到 {python}，请先运行 MyClaw-Setup.command")
        return
...
def main() -> None:
    try:
        ...
        print("或不使用菜单栏，直接双击 MyClaw.command 以后台方式运行。")
        sys.exit(1)

    start_server()

    class MyClawMenubar(rumps.App):
        def __init__(self):
            super().__init__("myclaw", title="◐", quit_button=None)
            self.menu = [
                rumps.MenuItem("查看状态", callback=self.on_status),
                rumps.MenuItem("打开日志", callback=self.on_log),
                None,
                rumps.MenuItem("退出 myclaw", callback=self.on_quit),
            ]
        ...
        def _status_detail(self) -> str:
            try:
                ...
                lines = [f"✅ MyClaw 后端运行正常 (端口 {SERVICE_PORT})"]
