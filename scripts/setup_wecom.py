...
注意：实测订阅会短暂挤掉同一机器人的既有长连接（若 MyClaw 已在运行
...
def main() -> int:
    print("MyClaw 企业微信配置向导")
    print(STEPS)

    if not ENV_PATH.exists():
        from_env = ""
        print(f"⚠️ 未找到 {ENV_PATH}，将新建（建议先运行 MyClaw-Setup.bat）")
    else:
        from_env = ""
    ...
    print("   （若 MyClaw 正在运行并已启用企微，旧连接会被本次测试短暂挤掉，服务会自动重连）")
    ...
    print("\n全部完成。请运行 MyClaw-Restart.bat 重启服务使企微通道生效，")
